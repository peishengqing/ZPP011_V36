...
## v42.15 — 审核后变更检测消噪（只盯实际数量）

**发布日期：2026-07-10**

### ✨ 新增功能
- 审核后变更检测改为「只盯实际数量」（方案A）：彻底消除 SAP 成本单价浮动/系统重估导致的偏差金额、偏差率集体误报；实测旧逻辑会误报全部行，现归零
- deviation_history 新增 field / old_qty / new_qty 字段，专门留痕「实际数量」变动（改前→改后）

### 🔧 问题修复
- 修复实际数量列识别：analyzer 主表把「数量-实际」重命名为「实际」，`_find_real_qty_col` 已同时覆盖两者，否则变更检测会静默失效（永远不报警）
- 修复标记已读/未读路径未建立实际数量基线：审核时保存 snapshot_qty，使后续有人改动实际数量能被可靠揪出

### ⚡ 优化
- 无

### 📌 备注
- 变更检测指纹不再绑定偏差金额|偏差率，只比对审核时实际数量基线
- 兼容性：升级前已读的旧记录 snapshot_qty 为 NULL，首次重分析时用当前数量静默建立基线，避免一次性全量误报
- 已通过实跑验证（5575 行真实数据）：全新加载误报=0、成本浮动重分析误报=0、改实际数量才报警=1、审核建立基线后改数量=1、deviation_history 留痕 field=实际数量
