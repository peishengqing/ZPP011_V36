...
def do_analysis_v2(
    ...
    # 优雅降级：处理缺少 金额-实际(含税) 列的情况（旧格式文件）
    if "金额-实际(含税)" not in df.columns:
        msg = "⚠️ 当前文件缺少[金额-实际(含税)]列（旧格式），含税偏差金额将按0处理"
        print(msg)
        df["金额-实际(含税)"] = 0.0
    ...
    headers5 = ['订单日期', '流程订单', '工厂', '车间', '物料类型', '原表行号',
                '物料编码', '物料名称', '单位', '定额', '实际',
                '偏差数量', '偏差率', '偏差金额', '备注', '备注来源', '偏差区间']
    rows5 = [[r['订单日期'], r.get('流程订单', ''), r['工厂'], r['车间'], r['物料类型'], r['原表行号'],
              r['物料编码'], r['物料名称'], r['单位'], r['定额'], r['实际'],
              r['偏差数量'], r['偏差率'], r['偏差金额'], r['备注'],
              r['备注来源'], r['偏差区间']] for r in dev_df.to_dict('records')]
