...
class ExportEvents:
    ...
    def generate_ppt(self):
        ...
        default_name = self.config.get("export.default_ppt_filename", "ZPP011偏差分析_{datetime}.pptx").format(datetime=datetime.datetime.now().strftime("%Y%m%d_%H%M")) + ".pptx"
        ...
        try:
            ...
            if "Permission denied" in error_msg or "PermissionError" in error_msg:
                ...
                _warn_msg = (
                    ...
                    "\u26a0\ufe0f 无法保存文件" + chr(10) + chr(10)


                    + "\u53ef\u80fd\u7684\u539f\u56e0\uff1a" + chr(10)


                    + "  • 文件已用 Excel 打开" + chr(10)


                    + "  • 文件被 WPS 或其他程序占用" + chr(10) + chr(10)


                    + "解决方法：" + chr(10)


                    + "  1. 关闭 Excel 中打开的这个文件" + chr(10)
                ...
                self.root.after(0, lambda: messagebox.showwarning("文件被占用", _warn_msg))


                self.root.after(0, lambda: self.log("\u26a0\ufe0f 文件被占用，请关闭 Excel 后重试", "warning"))
                ...
                self.root.after(0, lambda: self.log(f"\u274c 表格生成失败：{_err}", "error"))


            self.root.after(0, lambda: self.log(tb, "error"))


            self.root.after(0, lambda: self.excel_btn.configure(state="normal", text="\U0001f4cb 生成表格"))
    ...
    def _export_log(self):
        ...
        try:
            ...
            default_name = self.config.get("export.default_log_filename", "ZPP011日志_{ts}.txt").format(ts=ts)
    ...
    def _export_changelog(self):
        ...
        try:
            ...
            default_name = self.config.get("export.default_changelog_filename", "ZPP011版本日志_{ts}.txt").format(ts=ts)
