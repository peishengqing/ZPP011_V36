...
class TableEvents:
    ...
    def _init_scroll_binding(self):
        ...
        # 查找滚动条对象
        scrollbar = getattr(self, "audit_vscroll", None) or getattr(
            self, "vscroll", None
        )
        if scrollbar:
    ...
    def _rebuild_tag_cache_if_needed(self):
        ...
        if (
            not self._cache_built
            and self.audit_data is not None
            and not self.audit_data.empty
        ):
    ...
    def _refresh_audit_tree(self, df, skip_auto_sort=False):
        ...
        if df is not None and "material_category[2]" in df.columns:
            df.rename(
                columns={"material_category[2]": "material_category"}, inplace=True
            )
        # 重置分页状态
        ...
        if df is not None and not df.empty:
            ...
            _code_col = next(
                (c for c in ["物料编码", "组件物料号"] if c in df.columns), None
            )
            _type_desc_col = (
                "组件物料类型描述" if "组件物料类型描述" in df.columns else None
            )
            ...
            if _use_type_desc:
                ...
                def _get_mc(row):
                    ...
                    if _code_col:
                        ...
                        return (
                            _mc_map.get(code[:3], code[:3])
                            if pd.notna(row.get(_code_col)) and code != "nan"
                            else ""
                        )
                    return ""
            ...
            def _calc_priority_4color(row):
                ...
                has_note = (
                    pd.notna(row.get("备注原因")) and remark != ""
                ) or batch_remark != ""
                label = self._calculate_priority_label(
                    row.get("偏差率(%)", 0), has_note
                )
        ...
        for i, (idx, row) in enumerate(df.iterrows(), 1):
            ...
            if _mc_from_df and str(_mc_from_df).strip() not in (
                "",
                "nan",
                "NaN",
                "None",
            ...
            if cached:
                ...
                dir_tag = (
                    ("over_amount",)
                    if dev_rate > 0
                    else (("under_amount",) if dev_rate < 0 else ())
                )
                ...
                try:
                    if hasattr(self, "rule_engine"):
                        color_hex = self.rule_engine.get_color_for_deviation_rate(
                            dev_rate
                        )
                        try:
    ...
    def _show_audit_card(self, event):
        ...
        try:
            ...
            self._card_win.minsize(450, 500)  # 最小尺寸限制
    ...
    def _on_filter_changed(self, col_key):
        ...
        print(f"[DEBUG] _on_filter_changed: audit_data={len(self.audit_data)}行, filters={filters}")
        if '订单日期' in self.audit_data.columns:
            print(f"[DEBUG] 订单日期前3值: {self.audit_data['订单日期'].head(3).tolist()}")
        engine = FilterEngine()
        df_filtered = engine.apply(filters, self.audit_data)
        print(f"[DEBUG] _on_filter_changed: FilterEngine返回 {len(df_filtered)} 行")
    ...
    def _reset_all_filters(self):
        ...
        if hasattr(self, "status_filter_label"):
            ...
            self.status_filter_label.configure(text=f"📋 显示全部 | 共 {count} 条")
    ...
    def _export_views(self):
        ...
        default_name = (
            f"视图包_{datetime.now().strftime('%Y%m%d')}_v{get_current_version()}.json"
        )
    ...
    def _import_views(self):
        ...
        file_path = filedialog.askopenfilename(
            filetypes=[("JSON 文件", "*.json")], parent=self.root
        )
        if not file_path:
            return
        ...
        selected = self._multi_select_dialog(
            "选择要导入的视图", import_views_list, pre_check_conflicts=existing
        )
        if not selected:
            return
    ...
    def _multi_select_dialog(
        ...
        tk.Button(btn_frame_top, text="全选/取消", command=toggle_all).pack(
            side=tk.LEFT
        )
        ...
        tk.Button(
            btn_frame, text="确认", command=on_confirm, bg="#28a745", fg="white"
        ...
        if "material_category" in self.filter_widgets:
            ...
            if data_src is not None and "material_category" in data_src.columns:
                unique_cats = sorted(
                    [
                        str(v)
                        for v in data_src["material_category"].dropna().unique()
                        if v != ""
                    ]
                )
    ...
    def _update_audit_stats(self, filtered_data=None):
        ...
        if (
            data is None
            and hasattr(self, "filtered_data")
            and self.filtered_data is not None
            and len(self.filtered_data) > 0
        ):
        ...
        self.log(
            f"[DEBUG] _update_audit_stats: data={len(data)}行, 列={list(data.columns)[:10]}",
            "info",
        )
        ...
        if hasattr(self, "summary_cost_lbl"):
            try:
                ...
                if "数量-实际" in data.columns and "数量-定额" in data.columns:
                    qty_actual = pd.to_numeric(
                        data["数量-实际"], errors="coerce"
                    ).fillna(0)
                    qty_quota = pd.to_numeric(
                        data["数量-定额"], errors="coerce"
                    ).fillna(0)
                    total_qty_abs = (qty_actual - qty_quota).abs().sum()
                    unit_col = next(
                        (c for c in ["组件单位", "单位"] if c in data.columns), None
                    )
                    ...
                    if total_qty_abs > 0 and unit:
                        ...
                        if avg_price > 0:
                            ...
                            self.summary_cost_lbl.config(
                                text=f"💸 总偏差金额：¥{total_amount:,.2f}（无法换算实物量）"
                            )
                    else:
                        self.summary_cost_lbl.config(
                            text=f"💸 总偏差金额：¥{total_amount:,.2f}（无法换算实物量）"
        ...
        try:
            ...
            if amount_col:
                ...
                if amount_sum != 0:
                    # 计算总数量偏差
                    qty_actual_col = next(
                        (c for c in ["数量-实际", "实际"] if c in df.columns), None
                    )
                    qty_quota_col = next(
                        (c for c in ["数量-定额", "定额"] if c in df.columns), None
                    )
                    if qty_actual_col and qty_quota_col:
                        qty_actual = pd.to_numeric(
                            df[qty_actual_col], errors="coerce"
                        ).fillna(0)
                        qty_quota = pd.to_numeric(
                            df[qty_quota_col], errors="coerce"
                        ).fillna(0)
                        total_qty_abs = (qty_actual - qty_quota).abs().sum()
                        unit_col = next(
                            (c for c in ["组件单位", "单位"] if c in df.columns), None
                        )
    ...
    def _get_batch_summary(self, selected_items):
        ...
        unit_col = next(
            (c for c in ["组件单位", "单位"] if c in selected_df.columns), None
        )
        unit = (
            selected_df[unit_col].iloc[0] if unit_col and len(selected_df) > 0 else ""
        )
        if over_amount > 0:
            qty_actual = pd.to_numeric(
                selected_df.loc[selected_df["偏差金额"] > 0, "数量-实际"],
                errors="coerce",
            ).fillna(0)
            qty_quota = pd.to_numeric(
                selected_df.loc[selected_df["偏差金额"] > 0, "数量-定额"],
                errors="coerce",
            ).fillna(0)
