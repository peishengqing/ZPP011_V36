...
class ModeSelector:
    ...
    def __init__(self, parent):
        ...
        tk.Button(
            bottom,
            text="设置默认模式",
            command=self._set_default).pack(
            side="left")
...
class ZPP011Beautiful(EventsMixIn):
    ...
    def __init__(self, root):
        ...
        history_menu.add_command(
            label='历史对比', command=self._show_history_compare)

        history_menu.add_separator()

        history_menu.add_command(
            label='📊 管理看板',
            command=self._show_management_dashboard)

        history_menu.add_command(
            label='🔍 AI 归因分析',
            command=self._show_attribution_standalone)
        ...
        try:
            ...
            if os.path.exists(_defaults_path):
                ...
                _feature_enabled = _defaults.get(
                    'features', {}).get(
    ...
    def _on_sidebar_filter_changed(self, filters):
        ...
        if 'date_start' not in top_keys_used and 'date_end' not in top_keys_used:
            ...
            if hasattr(
                    self,
                    'filter_widgets') and 'order_date' in self.filter_widgets:
                ...
                if isinstance(date_widgets, tuple) and len(date_widgets) == 2:
                    ...
                    try:
                        ...
                        if date_widgets[0].get_date():
                            ...
                            filters['date_start'] = date_widgets[0].get_date().strftime(
                                '%Y-%m-%d')

                        if date_widgets[1].get_date():

                            filters['date_end'] = date_widgets[1].get_date().strftime(
                                '%Y-%m-%d')
    ...
    def _load_s01_display_config(self) -> dict:
        ...
        config_path = Path(__file__).parent.parent / \
            "config" / "s01_display.yaml"

        if not config_path.exists():

            self.log(
                f"⚠ S01 高亮配置不存在 {config_path}，使用内置默认规则",
                "warning") if hasattr(
                self,
                'log') else None
    ...
    def _s01_on_tab_changed(self, event=None):
        ...
        self.audit_data = self._tab_data_cache.get(
            current_tab, pd.DataFrame()).copy(deep=True)
    ...
    def _generate_excel_thread(self, output_path: str):
        ...
        try:
            ...
            if "Permission denied" in error_msg or "PermissionError" in error_msg:
                ...
                self.root.after(
                    0, lambda: self.log(
                        f"⚠️ 文件被占用，请关闭 Excel 后重试", "warning"))
            ...
            self.root.after(
                0, lambda: self.excel_btn.configure(
                    state="normal", text="📋 生成表格"))

            self.root.after(
                0, lambda: self.status_lbl.configure(
                    text="就绪", fg=C['text_dim']))
                ...
                code_cols = [
                    c for c in df.columns if any(
                        k in str(c).lower() for k in [
                            '组件物料号', '组件编码', '物料编码', 'code', '编码'])]

                name_cols = [
                    c for c in df.columns if c == '组件物料描述'] or [
                    c for c in df.columns if any(
                        k in str(c).lower() for k in [
                            '物料描述', '组件描述', '名称', 'name'])]
                ...
                if code_cols:
                    ...
                    for _, row in df.iterrows():
                        ...
                        if code and code != 'nan' and code not in seen:
                            ...
                            factory = str(row[factory_cols[0]]
                                          ) if factory_cols else ''
    ...
    def _analyze_trend(self):
        ...
        def calc_metrics(data):
            ...
            return {
                "偏差率均值": round(
                    dev_rate_mean, 1), "偏差金额合计": round(
                    dev_amount_sum, 2), "通过率": round(
                    pass_rate, 1)}
        ...
        try:
            ...
            if not trend:
                ...
                for period in ["早期", "中期", "近期"]:
                    ...
                    self.trend_labels[period]["dev_amount"].configure(
                        text="--")
            ...
            for period in ["早期", "中期", "近期"]:
                ...
                self.trend_labels[period]["dev_rate"].configure(
                    text=dev_rate_txt)

                self.trend_labels[period]["dev_amount"].configure(
                    text=dev_amount_txt)

                self.trend_labels[period]["pass_rate"].configure(
                    text=pass_rate_txt)
        ...
        if not output_path or not os.path.exists(output_path):
            ...
            for col in required_cols:
                ...
                if col not in dev_df.columns:
                    ...
                    results.append(
                        ('警告', f'关键列空值过多：{col} ({dev_df[col].isna().mean():.0%})'))
            ...
            if '备注' in dev_df.columns:
                ...
                no_note = dev_df['备注'].isna().sum(
                ) + (dev_df['备注'].astype(str).str.strip() == '').sum()

                note_rate = (len(dev_df) - no_note) / \
                    len(dev_df) if len(dev_df) > 0 else 0
            ...
            if all(c in dev_df.columns for c in dup_cols):
                ...
                if dup_count > 0:
                    ...
                    results.append(
                        ('警告', f'发现 {dup_count} 条同一订单重复物料记录（流程订单+物料编码）'))
                    ...
                    self._duplicate_records['_重复组'] = self._duplicate_records.groupby(
                        dup_cols).ngroup()
