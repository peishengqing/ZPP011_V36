"""
S01 模块：云南达利收发存汇总（筛选修复版）
修复：确保查询按钮触发重新查询，筛选条件正确传递
"""
...
class StockDatabase:
    ...
    def get_total_record_count(self):
        cursor = self.conn.cursor()
        cursor.execute("SELECT COUNT(*) FROM zmm062_summary")
        return cursor.fetchone()[0]

    def get_sample_months(self):
        cursor = self.conn.cursor()
        cursor.execute("SELECT DISTINCT report_month FROM zmm062_summary ORDER BY report_month LIMIT 3")
        return [row[0] for row in cursor.fetchall()]
...
def calculate_period_summary(df_raw, filters=None):
    """
    汇总收发存报表，返回筛选后数据及期末总金额
    （直接在 DataFrame 上操作，无需重新读 Excel）
    """
    if filters:
    ...
    # 使用英文列名（已通过 parse_zmm062_excel 映射）
    ending_amount_col = 'closing_amount'
    if ending_amount_col not in df_filtered.columns:
        raise KeyError(f"列 {ending_amount_col} 不存在，请检查数据")
    
    total_ending_amount = df_filtered[ending_amount_col].sum()
    
    return {
        'filtered_data': df_filtered,
        'total_ending_amount': total_ending_amount,
...
def parse_zmm062_excel(file_path):
    ...
    alias_map = {
        ...
        'plant_code': ['工厂'],
        'plant_name': ['工厂描述'],
        'storage_location': ['库存地点'],
        'storage_desc': ['库存地点描述'],
        'material_group': ['物料组描述', '物料组'],
        'material_code': ['物料号'],
        'material_desc': ['物料描述'],
        'spec_desc': ['规格描述'],
        'unit': ['单位'],
        'opening_qty': ['开始数量'],
        'unit_price': ['开始单价(含税)'],
        'opening_amount': ['开始金额(含税)'],
        'inbound_qty': ['入库总数量'],
        'inbound_amount': ['入库总金额(含税)'],
        'outbound_qty': ['出库总数量'],
        'outbound_amount': ['出库总金额(含税)'],
        'closing_qty': ['结束数量'],
        'closing_amount': ['结束金额(含税)'],
        'turnover_days': ['周转天'],
        'turnover_rate': ['周转率'],
        'group_warehouse_category': ['集团仓库分类']
    }
    mapping = {}
    ...
    for col in ['company_code', 'plant_code', 'plant_name', 'storage_location', 'storage_desc', 'material_group', 'material_desc', 'spec_desc', 'unit']:
...
class StockSummaryWindow:
    ...
    def _after_init(self):
        """初始化完成后加载数据（延迟执行，确保UI已完全创建）"""
        self.window.after(100, self._load_latest_month)
    ...
    def _build_ui(self):
        ...
        tk.Button(btn_frame, text="📂 导入 ZMM062 报表", command=self._import_excel,
                  bg=COLORS['accent'], fg="white", font=("Microsoft YaHei", 10, "bold"),
                  relief="flat", padx=10, pady=5).pack(side="left")
        tk.Button(btn_frame, text="🔄 重置筛选", command=self._reset_filters,
                  bg="#f39c12", fg="white", font=("Microsoft YaHei", 10),
                  relief="flat", padx=10, pady=5).pack(side="left", padx=(10,0))
    ...
    def _build_filter_bar(self, parent):
        ...
        self.plant_combo = ttk.Combobox(filter_frame, textvariable=self.plant_var, values=["全部", "1101", "1102"], state="readonly", width=15)
        self.plant_combo.grid(row=row, column=1, padx=5, pady=5, sticky="w")
        ...
        self.query_btn = tk.Button(btn_frame, text="查询", command=self._on_query_clicked, bg=COLORS['accent'], fg="white", width=8)
    ...
    def _build_table(self, parent):
        ...
        columns = ("report_month", "group_warehouse_category", "material_code", "material_desc", "unit",
                   "closing_qty", "closing_amount", "outbound_amount", "turnover_days", "flags")
        self.tree = ttk.Treeview(table_frame, columns=columns, show="headings", height=20)
        col_config = {
            "report_month": ("月份", 90, "center"),
    ...
    def _import_excel(self):
        ...
        try:
            ...
            rows_before = len(df)
            months_in_file = sorted(df['report_month'].unique())
            msg_pre = f"解析到 {rows_before} 行数据，月份：{', '.join(months_in_file)}\n"

            success_count = self.db.insert_summary(df)
            total_records = self.db.get_total_record_count()
            sample_months = self.db.get_sample_months()
            msg_post = f"成功插入 {success_count} 条记录。数据库中现有总记录数：{total_records}\n最近月份：{', '.join(sample_months)}"
            progress_win.destroy()
            messagebox.showinfo("导入成功", msg_pre + msg_post)
    ...
    def _on_query_clicked(self):
        """查询按钮的响应函数（确保每次点击都重新查询并刷新表格）"""
        # 修复：直接从 Combobox 控件获取当前选中值，避免 StringVar 不同步
        plant = self.plant_combo.get() if hasattr(self, 'plant_combo') else None
        category = self.category_combo.get() if hasattr(self, 'category_combo') else None
        mat_group = self.mat_group_combo.get() if hasattr(self, 'mat_group_combo') else None

        # 如果选中"全部"则转为 None
        plant = plant if plant and plant != "全部" else None
        category = category if category and category != "全部" else None
        mat_group = mat_group if mat_group and mat_group != "全部" else None
    ...
    def _update_stats_cards(self, df):
        ...
        # 修复：强制转换 closing_amount / outbound_amount 为数值（避免字符串导致 sum=0）
        df['closing_amount'] = pd.to_numeric(df['closing_amount'], errors='coerce').fillna(0)
        ...
        # 恢复状态栏显示记录数
        if 'report_month' in df.columns and not df.empty:
            last_month = df['report_month'].max()
            self.status_label.config(text=f"数据截止月份：{last_month} | 共 {len(df)} 条记录")
        else:
            self.status_label.config(text=f"共 {len(df)} 条记录")
    ...
    def _show_context_menu(self, event):
        ...
        menu.add_separator()
        menu.add_command(label="刷新", command=self._on_query_clicked)
    ...
    def _on_row_double_click(self, event):
        ...
        if item:
            values = self.tree.item(item, 'values')
            material_code = values[2] if len(values) > 2 else None
            if material_code:
                self.status_label.config(text=f"双击物料 {material_code}，联动功能待实现")

    def _reset_filters(self):
        # 修复二：重置时清空所有条件（包括月份），不回填
        if hasattr(self, 'plant_combo'):
            self.plant_combo.set("全部")
        if hasattr(self, 'category_combo'):
            self.category_combo.set("全部")
        if hasattr(self, 'mat_group_combo'):
            self.mat_group_combo.set("全部")
        self.start_month_var.set("")
        self.end_month_var.set("")
        self.search_var.set("")
        self._on_query_clicked()
