...
class AutoQuarantineRuleWidget(QWidget):
    ...
    def _known_categories(self):
        ...
        v43.98：直接使用数据中「组件物料类型描述」列的实际值，不添加臆造的分类。
        实测值：包材、原辅料、广宣、食品半成品、饮料半成品
        """
        cats, seen = [], set()
        # 数据中实际存在的组件物料类型描述值
        for c in ["包材", "原辅料", "广宣", "食品半成品", "饮料半成品"]:
            if c not in seen:
                seen.add(c)
                cats.append(c)
        # 从现有规则中收集已使用的类别值
    ...
    def _on_cat_toggled(self, state):
        ...
        # state 是 int 类型（0=Unchecked, 2=Checked），不能用 Qt.Checked 比较
        self.cat_checkbox_container.setEnabled(bool(state))
        if not state:
    ...
    def _on_unit_toggled(self, state):
        ...
        # state 是 int 类型（0=Unchecked, 2=Checked），不能用 Qt.Checked 比较
        self.unit_checkbox_container.setEnabled(bool(state))
        if not state:
