...
class ImportWizard(QWizard):
    ...
    def _add_pages(self):
        ...
        self.page_file.sheet_combo.currentTextChanged.connect(self._on_sheet_changed)
    ...
    def _build_preview(self):
        ...
        if self.import_type == 'alt':
            ...
            rows = [[item.get('工厂名称', ''), item.get('物料A编码', ''),
                     item.get('物料A名称', ''), item.get('物料B编码', ''),
                     item.get('物料B名称', '')] for item in self.parsed_data[:20]]
        else:
            headers = ['规则名称', '条件', '动作', '参数', '启用']
            rows = [[item.get('规则名称', ''), item.get('条件表达式', ''),
                     item.get('动作类型', ''), item.get('动作参数', ''),
                     item.get('启用', '')] for item in self.parsed_data[:20]]
