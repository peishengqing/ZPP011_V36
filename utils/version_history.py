...
VERSION_HISTORY = [
    {
        "version": "v42.15",
        "date": "2026-07-10",
        "build_datetime": "2026-07-10 12:33:00",
        "features": [
            "✦ 审核后变更检测改为「只盯实际数量」（方案A），彻底消除因 SAP 成本单价浮动/系统重估导致的偏差金额、偏差率集体误报（实测旧逻辑会误报全部行，现归零）",
        ],
        "fixes": [
            "🔧 修复实际数量列识别：analyzer 主表把「数量-实际」重命名为「实际」，已在 _find_real_qty_col 同时覆盖两者，否则变更检测会静默失效",
            "🔧 修复标记已读/未读路径未建立实际数量基线：审核时保存 snapshot_qty，使后续有人改动实际数量能被可靠揪出（旧记录无基线则首次重分析静默建基线迁移，不报警）",
        ],
        "optimizations": [],
        "notes": [
            "📌 变更检测指纹不再绑定偏差金额|偏差率，只比对审核时实际数量基线；deviation_history 新增 field/old_qty/new_qty 字段，留痕「实际数量」变动",
            "📌 兼容性：升级前已读的旧记录 snapshot_qty 为 NULL，首次重分析时用当前数量静默建立基线，避免一次性全量误报",
        ]
    },
    {
        "version": "v42.14",
