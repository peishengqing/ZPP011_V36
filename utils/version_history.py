...
VERSION_HISTORY = [
    {
        "version": "v42.0",
        "date": "2026-06-06",
        "build_datetime": "2026-06-06 01:00:00",
        "features": [
            "✨ 已读/未读标记 + 指纹比对 + 变动留痕（核心功能）",
            "✨ 替代料配对显示优化（紧凑2列 + 放大按钮）",
            "✨ 全局快捷键体系（F5分析/F6导出/F7报告/Ctrl+B标记已读/Ctrl+D复制备注）",
            "✨ Excel汇总统计预警列颜色填充（红/黄/绿）",
            "✨ 净偏差计算手动修正（临时方案）",
            "✨ 版本日志对话框（帮助 → 版本日志）"
        ],
        "fixes": [
            "🐛 修复偏差数量符号反转（删除多余反转代码）",
            "🐛 修复备注列为空（优先取备注列，其次备注原因）",
            "🐛 修复侧边栏筛选无效（日期筛选语法错误）",
            "🐛 修复表格行号不连续（重写headerData方法）",
            "🐛 修复净偏差计算不正确（重复代码+偏差金额获取逻辑）",
            "🐛 修复标题栏制作人位置（左边距调整到4px）",
            "🐛 修复Ctrl+B快捷键标记错误（改为标记选中行）"
        ],
        "optimizations": [
            "⚡ 预警列颜色显示（GUI表格红/黄/绿背景色）",
            "⚡ 标题栏颜色调整为更浅的蓝色（#42a5f5）",
            "⚡ 分析后不自动保存Excel（和tkinter版本一致）",
            "⚡ 数据库持久化（SQLite，跨session记住已读状态）"
        ],
        "notes": [
            "📌 本版本为 v42.0 预览版，包含多项核心功能",
            "📌 已读/未读功能需要SQLite支持（自动创建数据库 ~/.zpp011_audit/audit.db）",
            "📌 指纹比对功能可自动检测数据变动并强制设为未读",
            "📌 快捷键说明：F5=分析，F6=导出，F7=报告，Ctrl+B=标记已读，Ctrl+D=复制备注"
        ]
    },
    {
        "version": "v41.3",
