...
VERSION_HISTORY = [
    ...
    },
    {
        ...
        "fixes": [
            ...
            "场景：某行当初因「负损（实际<定额）」进入隔离区，后来补投使实际≥定额（相符/盘盈）或实际归零，其入区原因即已失效，但旧逻辑只增不删、会一直躺在隔离区。",
            "实现：隔离区表新增 reason_basis 列（入区判定依据快照）；scan_expired_quarantine 对每条仍存在于主表的隔离记录实时重判——负损类用实时实际/定额（相符/盘盈/归零细分），自动规则类重跑规则看是否还在命中集；手动非负损类无数据依据不翻标。",
