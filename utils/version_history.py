...
VERSION_HISTORY = [
    {
        ...
        "date": "2026-09-19",
        "features": "",
        "fixes": "自动已读规则4「饮料半成品负损」物料编码前缀由 400 更正为 410：饮料厂成品/瓶装物料编码实际以 410 开头（如 41000144 花生牛奶），400 基本是食品厂物料，原前缀在饮料厂语境下几乎空转、且与 v43.110 意图不符。现规则4 = 云南达利-饮料厂 + 名称不含箱/彩罐/不干胶/半成品 + 偏差量±10内 + 偏差率±10%内 + 410 前缀，自动已读饮料厂成品的小负损。仅改 config/auto_read_rules.json 单值，引擎逻辑不动。"
    },
    {
        "version": "v43.113",
        "date": "2026-09-15",
    ...
    },
    {
        "version": "v43.109",
        "date": "2026-09-18",
        "features": "主表新增 Excel 式列头筛选（方案A）：工具栏新增「🔽 列头筛选」开关按钮。开启后点列头弹出取值勾选浮层——搜索框 + 全选/清空 + 各取值勾选框（带行计数）+ 确定；取值取自源模型 DisplayRole，与表格实际显示字符串完全一致（含偏差率%后缀、(空)占位），勾选即成员过滤。已筛选列在表头左上角画橙色漏斗标。Ctrl+点列头仍走多级排序；开关关掉后点列头恢复排序行为。每次分析完成时统一清空取值过滤并清漏斗标。",
        "fixes": ""
    },
    {
        "version": "v43.108",
    ...
    },
    {
        ...
        "date": "2026-09-06",
        "features": "",
        "fixes": "自动隔离：修复半成品重分类列存在但全为空时无法回退到物料分类的问题。托盘（编码20000081，半成品重分类为空）现在能正确匹配「包材」规则进入隔离区。"
    },
    {
        "version": "v43.96",
        "date": "2026-09-06",
        "features": "",
        "fixes": "负损看板：移除半成品分类筛选中的虚拟项「食品半成品/饮料半成品」（这2个选项根本筛选不出数据），改为纯精确匹配。同步清理代码中无用的 blank/fac 变量及虚拟常量定义。"
    },
    {
        "version": "v43.95",
        "date": "2026-09-06",
    ...
    },
    {
        "version": "v43.93",
        "date": "2026-09-06",
        "features": "偏差率预警看板新增「偏差方向」筛选按钮组（全部/正偏差/负偏差），放在第1行隔离区之后。正偏差=偏差数量>0（超投），负偏差=偏差数量<0（少投）。数值列优先取「偏差数量」，缺失时回退「净偏差数量」/「偏差率(%)」，全缺失时等同不过滤。按钮带实时计数，与其他筛选组（已读/料别/隔离区/替代料/备注等）独立叠加。",
        "fixes": ""
    },
    {
        "version": "v43.92",
    ...
    },
    {
        ...
        "date": "2026-09-05",
        "features": "",
        "fixes": "负损看板物料类型下拉框过滤掉'食品成品'和'饮料成品'（这两个类型在负损数据中永远无记录，保留只会误导用户选了却筛出空表）。"
    },
    {
        "version": "v43.80",
        "date": "2026-09-05",
    ...
    },
    {
        ...
        "date": "2026-09-05",
        "features": "",
        "fixes": "调整负损看板默认筛选条件：包含未投料默认关（实际>0）、已读状态默认未读、隔离区默认否，与裴哥实际使用习惯一致。"
    },
    {
        "version": "v43.78",
        "date": "2026-09-05",
    ...
    },
    {
        ...
        "date": "2026-09-04",
        "features": "",
        "fixes": "负损看板工具栏放宽：各筛选组件最小/最大宽度从100-140扩至130-220，分隔间距从12扩至18，颜色复选框组由固定200px改为最小280px+固定高度36，整体不再拥挤，水平滚动条可正常使用。"
    },
    {
        "version": "v43.69",
        "date": "2026-09-04",
        "features": "",
        "fixes": "修复负损看板QGroupBox误用：半成品分类筛选从复选框组(QGroupBox+QVBoxLayout)改为QComboBox下拉列表，同时修复单位筛选回调方法签名。"
    },
    {
        "version": "v43.68",
        "date": "2026-09-04",
        "features": "负损看板优化：①关键字搜索默认清空；②包含未投料默认不勾选；③半成品分类和单位筛选由复选框组改为 QComboBox 下拉列表（不自动展开）；④单位筛选增加动态刷新（随其他筛选条件变化时，单位列表只保留当前可见数据的单位值）。",
        "fixes": "负损看板默认状态不符合直觉（关键字预填、未投料默认勾选）；半成品分类和单位筛选改为下拉框后缺少动态刷新逻辑。"
    },
    {
        "version": "v43.67",
        "date": "2026-09-04",
        "features": "负损看板新增单位动态复选框筛选：与半成品分类复选框组保持一致，默认全选，勾选多值 OR 逻辑。",
        "fixes": "负损看板缺少单位维度筛选能力。"
    },
    {
        "version": "v43.66",
        "date": "2026-09-02",
    ...
    },
    {
        ...
        "date": "2026-08-31",
        "features": "",
        "fixes": "负损看板车间筛选改为真正动态：_apply_filter 筛选数据后同步刷新车间下拉选项为当前可见数据的唯一值（保留当前选中项，若不在新列表则回退\"全部\"）。修复根因：主窗口传给负损看板的是全量主表df(_get_master_df)，set_data时unique()提取到所有车间而非筛选后的；现改为每次筛选后收缩车间选项，改关键词/料别等条件后车间下拉自动跟随更新。"
    },
    {
        "version": "v43.55",
        "date": "2026-08-31",
        "features": "",
        "fixes": "偏差率预警看板「车间」筛选从第2行(全屏才显示)提升至第1行始终可见，与已读/未读、工厂、隔离区并列。第1行隔离区之后追加车间下拉(全部/具体车间，动态从数据提取)；第2行现仅含料别+半成品分类(全屏时显示)。负损看板车间筛选本就在顶部始终可见且动态，无需改动。"
    },
    {
        "version": "v43.54",
        "date": "2026-08-30",
        "features": "",
        "fixes": "偏差率预警看板「隔离区」筛选从第3行(全屏才显示)提升至第1行始终可见，与已读/未读、工厂筛选并列(跟工厂筛选同样的处理)。第1行工厂筛选后追加隔离区下拉(全部/是/否)，删除第3行原隔离区块；第3行现仅含物料类型+替代料+是否备注(全屏时显示)。"
    },
    {
        "version": "v43.53",
        "date": "2026-08-30",
        "features": "",
        "fixes": "修复偏差率预警看板「隔离区」筛选失效：主表 data_service 已水合 _quarantined 列(0/1)，但传给看板的 candidates 未含该列，看板只能靠自构 data_id 临时比对隔离库，一旦「订单日期」类型/格式对不齐就全部漏判(筛选\"是\"永远0条)。改为：①主窗口 candidates 增加 _quarantined 列；②看板 set_data 优先用主表 _quarantined 列直接生成「隔离区」列(是/空)并 drop 内部列，data_id 比对降为兜底。让主表已有的隔离判断真正生效。"
    },
    {
        "version": "v43.52",
        "date": "2026-08-30",
        "features": "",
        "fixes": "修复变动提醒弹窗列错位：_populate 解析 data_id 时按 3 段硬编码(日期|流程订单|物料编码)，但实际 data_id 已升级为 4 段格式(工厂|日期|流程订单|物料编码)，导致工厂名被塞进日期列、日期被塞进流程订单列、流程订单号被塞进物料编码列。改为按段数自动适配 4 段/3 段格式。"
    },
    {
        "version": "v43.51",
        "date": "2026-08-30",
        "features": "",
        "fixes": "修复偏差率预警看板列错位：candidates 列表同时含「组件物料号」与「物料编码」两个来源列，两列并存时列顺序被打乱（表现为流程订单列显示日期、物料编码列显示流程订单号）。sheet5_full 已把组件物料号统一映射为物料编码，故移除冗余的「组件物料号」。负损看板 candidates 同步移除该冗余列，避免同类隐患。"
    },
    {
        "version": "v43.50",
        "date": "2026-08-29",
        "features": "",
        "fixes": "偏差率预警看板新增工厂筛选下拉框，动态从数据提取工厂唯一值，与车间筛选独立叠加生效"
    },
    {
        "version": "v43.49",
        "date": "2026-08-30",
    ...
    },
    {
        ...
        "date": "2026-08-30",
        "features": "",
        "fixes": "偏差率预警看板放大后筛选栏布局优化：改为多行 QVBoxLayout 布局（第1行已读+关键字、第2行料别+车间+半成品、第3行物料类型+隔离区+替代料+备注、第4行批量操作），全屏时自动展开所有行，还原时隐藏第二至四行"
    },
    {
        "version": "v43.46",
        "date": "2026-08-30",
    ...
    },
    {
        ...
        "date": "2026-08-29",
        "features": "",
        "fixes": "右键添加替代料配对时，工厂字段自动将'云南达利-食品厂'→'1101'、'云南达利-饮料厂'→'1102'，与手动添加格式一致"
    },
    {
        "version": "v43.43",
        "date": "2026-08-29",
    ...
    },
    {
        ...
        "date": "2026-08-29",
        "features": "",
        "fixes": "修复主表右键「添加为替代料配对」功能失效：extract函数中使用的英文列名(factory/code/name)与实际中文列名(工厂/物料编码/物料名称)不匹配，改为兼容多列名查找；负损看板新增备注筛选（全部/有/无）+ 压缩关键字搜索框宽度"
    },
    {
        "version": "v43.41",
        "date": "2026-08-29",
        "features": "",
        "fixes": "负损看板批量标记已读/未读性能优化：新增 _sync_main_df_batch 向量化批量更新，避免循环内逐行 at[] 赋值和多次 view_model.df 写入，解决一次性多标记时界面未响应问题"
    },
    {
        "version": "v43.40",
        "date": "2026-08-29",
        "features": "负损看板新增颜色筛选(复选框组对齐alert_dialog);新增车间筛选下拉框;新增隔离区筛选按钮(全部/是/否);隔离区列移至订单日期前",
        "fixes": ""
    },
    {
        "version": "v43.39",
    ...
    },
    {
        "version": "v43.37",
        "date": "2026-08-29",
        "features": "负损看板顶部添加「组件物料类型描述」筛选下拉框,支持按类型(包材/原料/半成品等)过滤",
        "fixes": ""
    },
    {
        "version": "v43.36",
        "date": "2026-08-29",
        "fixes": "自动隔离规则category_value匹配扩展:category_value=\"包材\"等通用分类值现在也匹配组件物料类型描述列(包含匹配),而不仅限于半成品重分类列的精确匹配;解决彩罐/托盘/瓶胚等包材物料因映射表无记录导致不进隔离区的bug",
        "features": "自动隔离匹配逻辑兼容半成品的权威映射表与包材等通用分类的关键词匹配"
    },
    {
        "version": "v43.35",
        "date": "2026-08-29",
        "features": "虚拟项「食品成品半成品」「饮料成品半成品」改为精确匹配+空白(裴哥反馈:综合组半成品/配料中心半成品不在筛选目标)",
        "fixes": "此前虚拟项用vals.str.contains('成品|半成品')模糊匹配,把「食品综合组半成品」「食品配料中心半成品」等也筛出来了;现改成(row_val==m)或(row_val=='')且工厂含对应前缀,只命中列值完全等于分类名或空白,排除其他含'半成品'字样的分类"
    },
    {
        "version": "v43.32",
        "date": "2026-08-29",
        "features": "虚拟项「食品成品半成品」「饮料成品半成品」模糊匹配扩展:空白(半成品重分类为空)且对应工厂含食品/饮料的记录也计入命中(裴哥口径:空白即成品半成品);数据列一字不动",
        "fixes": "此前虚拟项仅认列值含成品/半成品字样,空白记录筛不出;现偏差率/负损/告警三看板+主表proxy四处匹配逻辑统一把空白纳入虚拟项命中"
    },
    {
        "version": "v43.31",
        "date": "2026-08-29",
        "features": "主表左侧筛选面板「半成品分类」QComboBox单选→复选框组(全部+各原始值+虚拟两项「食品成品半成品」「饮料成品半成品」),与三看板对齐;虚拟项模糊匹配含成品/半成品且工厂匹配,满足主表成品半成品筛选需求",
        "fixes": "主表半成品分类筛选此前漏改(仅三看板v43.30改复选框),导致主表无法筛选成品半成品"
    },
    {
        "version": "v43.30",
        "date": "2026-08-29",
        "features": "规则编辑「要求属于类别」文本框改为复选框多选(已知分类勾选,多值OR);偏差率/负损/告警看板半成品分类筛选追加虚拟复选框「食品成品半成品」「饮料成品半成品」(模糊匹配含成品/半成品且工厂匹配)",
        "fixes": "回退v43.28全量覆盖:原逻辑把包材等映射值误覆盖为食品成品半成品,导致自动隔离规则category_value=包材匹配失效(彩罐高偏差不进隔离区);现恢复_empty_mask仅补空白,映射表原值不动"
    },
    {
        "version": "v43.29",
        "date": "2026-08-28",
        "optimizations": "偏差率预警看板搜索框与负损看板对齐：placeholder改为「关键字搜索(全列,逗号分隔多值OR)」+ tooltip;_keyword_mask支持逗号/顿号多值OR(原整框单关键词);re导入"
    },
    {
        "version": "v43.28",
        "date": "2026-08-28",
        "fixes": "半成品重分类成品仍空白：推断逻辑原本仅对空白值补位(被映射表填满后失效)；改为全量二次归并——凡满足「工厂含食品/饮料 + (物料分类=半成品 或 类型描述含半成品|成品)」的记录直接重分类为「食品成品半成品」/「饮料成品半成品」，确保下拉框可筛"
    },
    {
        "version": "v43.27",
        "date": "2026-08-28",
    ...
    {
        "date": "2026-08-28",
        "fixes": "主界面默认最大化真凶修复：删除showEvent里QTimer.singleShot(0,showMaximized)干扰（它在app.exec()事件循环中覆盖了Win32 API ShowWindow调用）；Win32 ShowWindow延迟100ms双保险"
    },
    {
        "version": "v43.22",
        "date": "2026-08-28",
